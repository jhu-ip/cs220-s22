...
2.	Another subdirectory *cs220-f21-public*, cloned from the public, coursewide repo.

If this is not the case, ask for assistance.  You may need to redo steps from earlier exercises. 

### Part 2

1.	Type `cd cs220-f21-public` to navigate into your working copy of the remote, coursewide public repo.
...
`ls exercises/ex2-2` -- you should see three `.c` source files listed.

### Part 3
1.	We created a `.bashrc` unix shell file that contains several alias and environment settings that will make your ugrad server lives better. Along with this, we have a `.bash_profile` configuration file which will get the `.bashrc` settings into place when you log in remotely. While you are still in the *cs220-f21-public* directory, type `ls -a` to see this (hidden) file, along with the usual suspects (i.e., `exercises` folder).
...
3.	Make a new directory for today’s exercise by typing `mkdir ex2-2`.

4.	Change to the new directory with `cd ex2-2`.

5.	Copy the three template source files to the current directory using: `cp ~/cs220-f21-public/exercises/ex2-2/*.c .` (Don't forget the space-then-dot at the end of the line above! Also, remember to insert any subdirectories you may have created into the path above.)

### Part 5
The three files that you just copied to your *~/cs220/ex2-2* directory are named *count1.c*, *count2.c* and *count3.c*.  Each of the files is an incomplete program that you must complete and test yourself.  Starting with *count1.c*:
...
$ git commit -m "ex2-2 count1 working version"
